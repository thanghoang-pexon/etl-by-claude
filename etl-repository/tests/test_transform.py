...
class TestNbaPipelineTransform:
    def test_columns_renamed_to_lowercase(self):
        df = pipeline.transform(make_nba_trio())
    ...
    def test_efficiency_formula(self):
        ...
        expected = round(
            ...
            + row["stl"] * 2.0 + row["blk"] * 2.0 - row["tov"], 2,
        )
...
class TestValidate:
    def test_valid_dataframe_passes(self):
        df = pipeline.transform(make_raw_df(n=100))
        result = validate(df)
...
class TestRegistry:
    def test_get_pipeline_nba(self):
        from pipelines import get_pipeline
        p = get_pipeline("nba_stats")
        assert p.name == "nba_stats"
        assert p.table_name == "nba_player_stats"

    def test_get_pipeline_unknown_raises(self):
        from pipelines import get_pipeline
        with pytest.raises(ValueError, match="Unbekannte Pipeline"):
            get_pipeline("does_not_exist")

    def test_pipeline_has_required_interface(self):
        from pipelines import get_pipeline
        p = get_pipeline("nba_stats")
        assert callable(p.extract)
        assert callable(p.transform)
        assert callable(p.validate)
        assert callable(p.create_table_sql)
